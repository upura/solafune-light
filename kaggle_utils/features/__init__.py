...
def merge_columns_with_mutual_info_score(dataframe, columns, threshold=0.3):
    for c1, c2 in combinations(columns, 2):
        if normalized_mutual_info_score(dataframe[c1],
                                        dataframe[c2],
                                        average_method='arithmetic') > threshold:
...
def target_encoding(train: pd.DataFrame, test: pd.DataFrame, encode_col, target_col, cv):
